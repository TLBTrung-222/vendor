  ...
  useEffect(() => {
    ...
    const getDocumentStatus = () => {
        return documentTypes.every((type) => {
            ...
            return (
                doc !== undefined && doc.document_status?.title === "Approved"
            );
        });
    };
        ...
        const fetchVendorIdByEmail = async () => {
            ...
            try {
                ...
                if (result.data) {
                    ...
                    if (
                        result.data.onboarding_transaction.stage_1
                            .status_description
                    ) {
                        setOnboardingStatus(
                            result.data.onboarding_transaction.stage_1
                        ...
                        setPmName(
                            result.data.onboarding_transaction.stage_1.pm_name
                        );
                        setUpdateDate(
                            new Date(
                    ...
                    if (result.data.postcodes) {
                        const postcodes = result.data.postcodes.map(
                            (item: any) => ({
                                code: item.postcode,
                                radius: item.radius,
                            })
                    ...
                    if (result.data.contact_user) {
                        ...
                        if (result.data.contact_user.position) {
                            setSelectedPosition(
                                result.data.contact_user.position.title
                            );
                        }
                    }
        ...
        const tmp = postcodeList
            ...
            .filter(
                (item, index, self) =>
                    index ===
                    self.findIndex(
                        (t) => t.code === item.code && t.label === item.label
                    )
            );
        setPostalCodes(tmp);
    }, []);
    ...
    const handleCancel = () => {
        setOnboardingStatus(
            vendorDetails?.onboarding_transaction?.stage_1
                ?.status_description || ""
        );
        setPmName(
            vendorDetails?.onboarding_transaction?.stage_1?.pm_name || ""
        );
        setUpdateDate(
            new Date(
            ...
            >
                ...
                >
                    ...
                    <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                        Are you sure you want to move to the following step?
                            ...
                            <Grid item xs={12}>
                                ...
                                >
                                    <TabContext value={region}>
                                        ...
                                        <TabPanel value="2">
                                            <Typography sx={{ mb: 2 }}>
                                                If you operate in specific
                                                postal code areas, switch to the
                                                Postcode tab.
                                            </Typography>
                                            <Box
                                                sx={{
